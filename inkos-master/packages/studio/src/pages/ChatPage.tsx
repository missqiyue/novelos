...
export function ChatPage({ activeBookId, nav, theme, t, sse: _sse }: ChatPageProps) {
  ...
  const [globalEnvModel, setGlobalEnvModel] = useState<string | null>(null);

  useEffect(() => { void fetchServices(); }, [fetchServices]);
  useEffect(() => {
    let cancelled = false;
    fetch("/api/v1/services/config")
      .then((r) => r.json())
      .then((data: any) => {
        if (cancelled) return;
        const model = data?.envConfig?.global?.model;
        setGlobalEnvModel(typeof model === "string" && model.trim().length > 0 ? model.trim() : null);
      })
      .catch(() => {});
  ...
  const modelPickerStatus = useMemo(() => {
    if ((servicesLoading || services.length === 0) && !globalEnvModel) return "loading" as const;
    const connected = services.filter((s) => s.connected);
    if (connected.length === 0) return globalEnvModel ? "ready" as const : "no-models" as const;
    ...
    return connected.some((s) => (modelsByService[s.service]?.models.length ?? 0) > 0)
      ? "ready" as const
      : globalEnvModel ? "ready" as const : "no-models" as const;
  }, [services, servicesLoading, modelsByService, globalEnvModel]);
  ...
  const groupedModels = useMemo(() => {
    ...
    return globalEnvModel
      ? [{ service: "__env__", label: "全局 ~/.inkos/.env", models: [{ id: globalEnvModel }] }, ...groups]
      : groups;
  }, [services, modelsByService, globalEnvModel]);
