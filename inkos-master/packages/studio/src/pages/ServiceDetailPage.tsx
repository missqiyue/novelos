...
export function ServiceDetailPage({ serviceId, nav }: { serviceId: string; nav: Nav }) {
  ...
  const handleDelete = async () => {
    ...
    const ok = confirm(`确认删除服务商“${label}”吗？这将同时移除配置与密钥。`);
    if (!ok) return;
    setStatus({ state: "saving" });
    try {
      await fetchJson(`/services/${encodeURIComponent(serviceId)}`, { method: "DELETE" });
      await refreshServices();
      nav.toServices();
    } catch (e) {
        ...
        <div className="flex items-center gap-2">
          ...
          {serviceId.startsWith("custom:") && (
            <button
              ...
              disabled={isBusy}
              className="flex items-center gap-1.5 px-3.5 py-2 text-xs rounded-lg border border-destructive/50 text-destructive hover:bg-destructive/10 transition-colors disabled:opacity-50"
            >
              删除
            </button>
          )}
