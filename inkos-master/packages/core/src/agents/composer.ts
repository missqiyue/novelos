...
export class ComposerAgent extends BaseAgent {
  ...
  private async collectSelectedContext(
    ...
    let ragEntries: ContextPackage["selectedContext"] = [];
    try {
      const ragResults = await this.retrieveRagContext(
        dirname(storyDir),
        plan.intent.goal,
        plan.intent.chapter,
      );
      ragEntries = ragResults.map((res: any) => ({
        source: res.source_file || `rag/chunk/${res.chunk_id}`,
        reason: "RAG retrieved semantic context (RRF + Time Decay)",
        excerpt: res.content,
      }));
    } catch (err) {
      this.ctx.logger?.warn?.(`RAG retrieval failed: ${err}`);
    }
  ...
  private async retrieveRagContext(bookDir: string, goal: string, chapterNumber: number): Promise<any[]> {
    const ragUrl = process.env.RAG_API_URL || "http://localhost:8088";
    const res = await fetch(`${ragUrl}/api/v1/retrieve/context`, {
      method: "POST",
    ...
    if (!res.ok) {
      throw new Error(`RAG request failed with status ${res.status}`);
    }
