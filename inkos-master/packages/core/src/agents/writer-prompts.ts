...
export function buildWriterSystemPrompt(
  ...
  const sections = isEnglish
    ...
    : [
        ...
        !governed ? buildAntiAIRewriteGuide() : "",
        !governed ? buildWebnovelAttractionGuide() : "",
...
function buildAntiAIRewriteGuide(): string {
  return `## 去AI味改写指南（骨架版｜可执行清单）
...
function buildWebnovelAttractionGuide(): string {
  return `## 网文吸引力写作指南

> 目标：解决剧情拖沓、逻辑性太强、缺少吸引力的问题。小说是让读者"感受"而不是"理解"。

### 三个"不要"
- **不要流水账**：读者会跳过，每个场景必须有冲突或情感。
- **不要逻辑推理**：读者不关心纯逻辑，用情感和悬念推动剧情。
...
### 场景与心理描写技巧
- **五感法**：每场景至少用3种感官（视觉/听觉/嗅觉/触觉/味觉）。
- **心理外化**：用动作暗示（拳头握紧）、感官反应（冷汗浸透）、对比反差（嘴硬手抖）代替直白的"他感到很愤怒"。
...
### 情感渲染与悬念设计
- **情感渲染三步**：铺垫（环境/动作暗示） → 爆发（情感高潮） → 余韵（反应/后果）。
- **悬念层级**：章末必有大悬念（新威胁/新秘密/新代价/反转），场景内自然分布中悬念。`;
}
