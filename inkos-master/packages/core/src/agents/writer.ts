...
export class WriterAgent extends BaseAgent {
  ...
  async saveChapter(
    ...
    const existingFiles = files.filter((f) => f.startsWith(paddedNum + "_") && f.endsWith(".md"));
    for (const f of existingFiles) {
      if (f !== filename) {
        const runtimeDir = join(storyDir, "runtime");
        await mkdir(runtimeDir, { recursive: true });
        const stamp = new Date().toISOString().replace(/[:.]/g, "-");
        const backupName = `chapter-${paddedNum}.${stamp}.md`;
        const backupPath = join(runtimeDir, backupName);
        const oldContent = await readFile(join(chaptersDir, f), "utf-8").catch(() => "");
        if (oldContent) {
          await writeFile(backupPath, oldContent, "utf-8");
        }
        await rm(join(chaptersDir, f), { force: true });
      }
    }
