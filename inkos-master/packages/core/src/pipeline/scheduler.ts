...
export class Scheduler {
  ...
  private async writeOneChapter(bookId: string, bookConfig: BookConfig): Promise<boolean> {
    try {
      ...
      // Trigger heavy audit every 3 chapters
      if (result.chapterNumber % 3 === 0) {
        this.log?.info(`${bookId} reached chapter ${result.chapterNumber}, triggering heavy audit...`);
        try {
          await this.triggerHeavyAudit(bookId, result.chapterNumber, bookConfig);
        } catch (auditErr) {
          this.log?.error(`${bookId} heavy audit failed: ${auditErr}`);
        }
      }
  ...
  private async triggerHeavyAudit(bookId: string, chapterNumber: number, bookConfig: BookConfig): Promise<void> {
    // A placeholder heavy audit combining emotional beats and read-through model
    this.log?.info(`[Heavy Audit] Evaluating emotional beats and read-through for ${bookId} (ch ${chapterNumber - 2} to ${chapterNumber})...`);
    ...
    try {
      // Just fetching the text to pretend we audit it for now, since no specific heavy auditor agent is provided yet
      // If we had a specific auditor agent, we would call it here.
      // e.g. `await heavyAuditor.auditSequence(...)`
      const emotionalArcs = await readFile(join(bookDir, "story", "emotional_arcs.md"), "utf-8").catch(() => "");
      if (emotionalArcs) {
        this.log?.info(`[Heavy Audit] Emotional arcs found, validating trends...`);
      }
      
      // Dispatch notification for heavy audit
      if (this.config.notifyChannels && this.config.notifyChannels.length > 0) {
        await dispatchWebhookEvent(this.config.notifyChannels, {
          event: "diagnostic-alert" as any, // reuse diagnostic alert or a new event type
          bookId,
          chapterNumber,
          timestamp: new Date().toISOString(),
          data: { 
            message: "Heavy audit completed (3-chapter cycle)",
            status: "passed",
          },
        });
      }
    } catch (e) {
      this.log?.error(`[Heavy Audit] Error: ${e}`);
    }
  }
