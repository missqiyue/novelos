...
import { PipelineSnapshotDB } from "../state/pipeline-snapshot.js";
...
export class PipelineRunner {
  ...
  private readonly pipelineDb: PipelineSnapshotDB;
  private memoryIndexFallbackWarned = false;

  constructor(config: PipelineConfig) {
    this.config = config;
    this.state = new StateManager(config.projectRoot);
    this.pipelineDb = new PipelineSnapshotDB(config.projectRoot);
  }
  ...
  async reviseDraft(bookId: string, chapterNumber?: number, mode: ReviseMode = DEFAULT_REVISE_MODE): Promise<ReviseResult> {
    ...
    try {
      ...
      const sanitizedTitle = chapterMeta.title.replace(/[/\\?%*:|"<>]/g, "").replace(/\s+/g, "_").slice(0, 50);
      const newFilename = `${paddedNum}_${sanitizedTitle}.md`;
      ...
      if (existingFile !== newFilename) {
        ...
        const backupName = `chapter-${paddedNum}.revise.${stamp}.md`;
        const backupPath = join(runtimeDir, backupName);
        const oldContent = await readFile(join(chaptersDir, existingFile), "utf-8").catch(() => "");
        if (oldContent) {
  ...
  private async _writeNextChapterLocked(bookId: string, wordCount?: number, temperatureOverride?: number): Promise<ChapterPipelineResult> {
    ...
    // Load pipeline snapshot if exists
    const snapshot = this.pipelineDb.getSnapshot(bookId, chapterNumber);
    let currentStage = snapshot?.stage ?? "start";
    let stateData = snapshot ? JSON.parse(snapshot.stateData) : {};
    
    if (snapshot) {
      this.logStage(stageLanguage, { zh: `检测到断点记录，从 ${currentStage} 阶段恢复`, en: `Breakpoint detected, resuming from ${currentStage} stage` });
    }

    let writeInput = stateData.writeInput;
    if (!writeInput || currentStage === "start") {
      this.logStage(stageLanguage, { zh: "准备章节输入", en: "preparing chapter inputs" });
      ...
      stateData.writeInput = writeInput;
      currentStage = "write_draft";
      this.pipelineDb.saveSnapshot(bookId, chapterNumber, currentStage, stateData);
    }
    ...
    // 1. Write chapter
    let output = stateData.output;
    if (!output || currentStage === "write_draft") {
      const writer = new WriterAgent(this.agentCtxFor("writer", bookId));
      this.logStage(stageLanguage, { zh: "撰写章节草稿", en: "writing chapter draft" });
      ...
      stateData.output = output;
      currentStage = "review_cycle";
      this.pipelineDb.saveSnapshot(bookId, chapterNumber, currentStage, stateData);
    }
    const writerCount = countChapterLength(output.content, lengthSpec.countingMode);
    ...
    let reviewResult = stateData.reviewResult;
    if (!reviewResult || currentStage === "review_cycle") {
      reviewResult = await runChapterReviewCycle({
      ...
      stateData.reviewResult = reviewResult;
    ...
    const writerAgent = new WriterAgent(this.agentCtxFor("writer", bookId));
    ...
    await persistChapterArtifacts({
      ...
      saveChapter: () => writerAgent.saveChapter(bookDir, persistenceOutput, gp.numericalSystem, pipelineLang),
      saveTruthFiles: async () => {
        await writerAgent.saveNewTruthFiles(bookDir, persistenceOutput, pipelineLang);
        ...
        this.logStage(stageLanguage, { zh: "同步 RAG 向量索引", en: "syncing RAG vector indexes" });
        await this.syncRagIndex(bookDir, chapterNumber, finalContent, persistenceOutput.chapterSummary).catch((err) => {
          this.logWarn(stageLanguage, {
            zh: `RAG 向量索引同步失败: ${err}`,
            en: `RAG vector index sync failed: ${err}`,
          });
        });
      },
    ...
    this.pipelineDb.clearSnapshot(bookId, chapterNumber);
...
${matrix}`,
  ...
  async importChapters(input: ImportChaptersInput): Promise<ImportChaptersResult> {
    ...
    try {
      ...
      for (let i = startFrom - 1; i < input.chapters.length; i++) {
        ...
        try {
          log?.info(this.localize(resolvedLanguage, {
            zh: `同步章节 ${chapterNumber} RAG 向量索引...`,
            en: `Syncing chapter ${chapterNumber} RAG vector index...`,
          }));
          await this.syncRagIndex(bookDir, chapterNumber, ch.content, output.chapterSummary);
        } catch (err) {
          log?.warn(this.localize(resolvedLanguage, {
            zh: `章节 ${chapterNumber} RAG 向量索引同步失败: ${err}`,
            en: `Chapter ${chapterNumber} RAG vector index sync failed: ${err}`,
          }));
        }
  ...
  private async syncRagIndex(bookDir: string, chapterNumber: number, content: string, summary?: string): Promise<void> {
    const ragUrl = process.env.RAG_API_URL || "http://localhost:8088";
    ...
    // A simple chunking by roughly 500-1000 characters per scene if no explicit scene breaks
    for (const p of paragraphs) {
      if (p.includes("***") || p.includes("---") || p.includes("===")) {
        if (currentSceneText) {
          scenes.push({ index: sceneIdx++, content: currentSceneText.trim() });
          currentSceneText = "";
        }
      } else {
        currentSceneText += p + "\n\n";
        if (currentSceneText.length > 800) {
          scenes.push({ index: sceneIdx++, content: currentSceneText.trim() });
          currentSceneText = "";
        }
      }
    ...
    const res = await fetch(`${ragUrl}/api/v1/index/chapter`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        project_root: bookDir,
    ...
    if (!res.ok) {
      throw new Error(`RAG index failed with status ${res.status}`);
    }
  }
