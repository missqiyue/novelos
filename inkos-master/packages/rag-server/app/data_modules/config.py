...
def _load_dotenv():
    ...
    约定：
    - 项目级 `.env`（当前工作目录下）优先；
    - 全局 `.env` 作为兜底：`~/.inkos/.env`，以及旧版 `~/.claude/webnovel-writer/.env`
    """
    # 1) 当前目录（常见：用户从项目根目录执行）
    _load_dotenv_file(Path.cwd() / ".env", override=False)

    # 2) 新版全局（用户要求使用的 ~/.inkos/.env）
    inkos_env = Path.home() / ".inkos" / ".env"
    _load_dotenv_file(inkos_env, override=False)
...
class DataModulesConfig:
    ...
    embed_base_url: str = field(default_factory=lambda: os.getenv("EMBED_BASE_URL", os.getenv("INKOS_EMBED_BASE_URL", os.getenv("INKOS_LLM_BASE_URL", "https://api-inference.modelscope.cn/v1"))))
    embed_model: str = field(default_factory=lambda: os.getenv("EMBED_MODEL", os.getenv("INKOS_EMBED_MODEL", "Qwen/Qwen3-Embedding-8B")))
    embed_api_key: str = field(default_factory=lambda: os.getenv("EMBED_API_KEY", os.getenv("INKOS_EMBED_API_KEY", os.getenv("INKOS_LLM_API_KEY", ""))))
    ...
    rerank_base_url: str = field(default_factory=lambda: os.getenv("RERANK_BASE_URL", os.getenv("INKOS_RERANK_BASE_URL", os.getenv("INKOS_LLM_BASE_URL", "https://api.jina.ai/v1"))))
    rerank_model: str = field(default_factory=lambda: os.getenv("RERANK_MODEL", os.getenv("INKOS_RERANK_MODEL", "jina-reranker-v3")))
    rerank_api_key: str = field(default_factory=lambda: os.getenv("RERANK_API_KEY", os.getenv("INKOS_RERANK_API_KEY", os.getenv("INKOS_LLM_API_KEY", ""))))
    ...
    context_ranker_recency_weight: float = field(default_factory=lambda: float(os.getenv("CONTEXT_RANKER_RECENCY_WEIGHT", "0.7")))
    context_ranker_frequency_weight: float = field(default_factory=lambda: float(os.getenv("CONTEXT_RANKER_FREQUENCY_WEIGHT", "0.3")))
