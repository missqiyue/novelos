...
pub fn start_outline_revision(
    ...
    if !valid_types.contains(&target_type.as_str()) {
        return Err(format!("Invalid target_type: '{}'. Must be one of: volume_outline, chapter_outline, arc", target_type));
    }
...
pub fn confirm_outline_revision(
    ...
    let impact = analyze_outline_revision_impact_with_conn(conn, volume_id, chapter_start, chapter_end)?;
    ...
    crate::commands::ledger::notify_pipeline_event(
        conn, &project_id, "outline_revision", "info",
        &format!("大纲修正：{} - 影响{}章", change_description, propagated.len()),
    );
