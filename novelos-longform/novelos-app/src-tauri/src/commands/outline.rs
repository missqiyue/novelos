...
#[tauri::command]
pub fn save_book_outline(db: State<'_, DbState>, content_json: String, change_reason: Option<String>) -> Result<BookOutlineInfo, String> {
    let project_conn = db.project.lock().map_err(|e| e.to_string())?;
    let conn = project_conn.as_ref().ok_or("No project open")?;
    ...
    let next_version: i64 = conn
        .query_row("SELECT COALESCE(MAX(version), 0) + 1 FROM book_outlines", [], |r| r.get(0))
        .unwrap_or(1);
...
#[tauri::command]
pub fn save_volume_outline(db: State<'_, DbState>, volume_id: String, content_json: String, change_reason: Option<String>) -> Result<VolumeOutlineInfo, String> {
    let project_conn = db.project.lock().map_err(|e| e.to_string())?;
    let conn = project_conn.as_ref().ok_or("No project open")?;
    ...
    let next_version: i64 = conn
        .query_row("SELECT COALESCE(MAX(version), 0) + 1 FROM volume_outlines WHERE volume_id = ?1", [&volume_id], |r| r.get(0))
        .unwrap_or(1);
...
#[tauri::command]
pub fn save_chapter_outline(db: State<'_, DbState>, chapter_number: i64, content_json: String, task_id: Option<String>, change_reason: Option<String>) -> Result<ChapterOutlineInfo, String> {
    let project_conn = db.project.lock().map_err(|e| e.to_string())?;
    let conn = project_conn.as_ref().ok_or("No project open")?;
    ...
    let next_version: i64 = conn
        .query_row("SELECT COALESCE(MAX(version), 0) + 1 FROM chapter_outlines WHERE chapter_number = ?1", [chapter_number], |r| r.get(0))
        .unwrap_or(1);
...
#[tauri::command]
pub fn update_volume(db: State<'_, DbState>, id: String, title: Option<String>, goal: Option<String>, main_conflict: Option<String>, climax: Option<String>, settlement: Option<String>, status: Option<String>) -> Result<(), String> {
    let project_conn = db.project.lock().map_err(|e| e.to_string())?;
    let conn = project_conn.as_ref().ok_or("No project open")?;

    if let Some(v) = title {
        conn.execute("UPDATE volumes SET title = ?1 WHERE id = ?2", rusqlite::params![v, id]).map_err(|e| e.to_string())?;
    }
    if let Some(v) = goal {
        conn.execute("UPDATE volumes SET goal = ?1 WHERE id = ?2", rusqlite::params![v, id]).map_err(|e| e.to_string())?;
    }
    if let Some(v) = main_conflict {
        conn.execute("UPDATE volumes SET main_conflict = ?1 WHERE id = ?2", rusqlite::params![v, id]).map_err(|e| e.to_string())?;
    }
    if let Some(v) = climax {
        conn.execute("UPDATE volumes SET climax = ?1 WHERE id = ?2", rusqlite::params![v, id]).map_err(|e| e.to_string())?;
    }
    if let Some(v) = settlement {
        conn.execute("UPDATE volumes SET settlement = ?1 WHERE id = ?2", rusqlite::params![v, id]).map_err(|e| e.to_string())?;
    }
    if let Some(v) = status {
        conn.execute("UPDATE volumes SET status = ?1 WHERE id = ?2", rusqlite::params![v, id]).map_err(|e| e.to_string())?;
    }
...
#[tauri::command]
pub fn list_arcs(db: State<'_, DbState>, volume_id: Option<String>) -> Result<Vec<ArcInfo>, String> {
    let project_conn = db.project.lock().map_err(|e| e.to_string())?;
    let conn = project_conn.as_ref().ok_or("No project open")?;
    ...
    let items = stmt.query_map(rusqlite::params![volume_id], |row| {
        Ok(ArcInfo {
            id: row.get(0)?, volume_id: row.get(1)?, arc_type: row.get(2)?, title: row.get(3)?,
            chapter_start: row.get(4)?, chapter_end: row.get(5)?, goal: row.get(6)?, status: row.get(7)?, priority: row.get(8)?,
        })
    }).map_err(|e| e.to_string())?
    .collect::<Result<Vec<_>, _>>()
    .map_err(|e| e.to_string())?;
...
pub fn analyze_outline_impact(
    ...
    let (start, end) = if let Some(vid) = &volume_id {
        let (s, e): (Option<i64>, Option<i64>) = conn.query_row(
            "SELECT chapter_start, chapter_end FROM volumes WHERE id = ?1", [vid],
    ...
    let affected_chapters: Vec<i64> = stmt.query_map([start, end], |row| row.get(0))
        .map_err(|e| e.to_string())?.collect::<Result<Vec<_>, _>>().map_err(|e| e.to_string())?;

    let mut stmt = conn.prepare(
        "SELECT volume_number FROM volumes WHERE chapter_end >= ?1 AND chapter_start <= ?2"
    ).map_err(|e| e.to_string())?;
    let affected_volumes: Vec<i64> = stmt.query_map([start, end], |row| row.get(0))
        .map_err(|e| e.to_string())?.collect::<Result<Vec<_>, _>>().map_err(|e| e.to_string())?;
    ...
    let affected_characters: Vec<String> = stmt.query_map([start, end], |row| row.get(0))
        .map_err(|e| e.to_string())?.collect::<Result<Vec<_>, _>>().map_err(|e| e.to_string())?;

    let mut stmt = conn.prepare(
        "SELECT title FROM foreshadow_items WHERE seed_chapter BETWEEN ?1 AND ?2 OR (resolved_chapter BETWEEN ?1 AND ?2)"
    ).map_err(|e| e.to_string())?;
    let affected_foreshadows: Vec<String> = stmt.query_map([start, end], |row| row.get(0))
        .map_err(|e| e.to_string())?.collect::<Result<Vec<_>, _>>().map_err(|e| e.to_string())?;
    ...
    let risk_level = if finalized_in_range > 5 { "high" } else if finalized_in_range > 2 { "medium" } else { "low" };
    let mut suggestions = Vec::new();
    if finalized_in_range > 0 { suggestions.push(format!("{}章已定稿，修改后需要重新审阅", finalized_in_range)); }
    if affected_foreshadows.len() > 3 { suggestions.push("修改范围内包含多个伏笔，需检查连续性".to_string()); }

    Ok(OutlineImpactReport { affected_volumes, affected_chapters, affected_characters, affected_foreshadows, risk_level: risk_level.to_string(), suggestions })
}
...
#[tauri::command]
pub fn list_event_nodes(db: State<'_, DbState>, arc_id: Option<String>) -> Result<Vec<EventNodeInfo>, String> {
    let project_conn = db.project.lock().map_err(|e| e.to_string())?;
    let conn = project_conn.as_ref().ok_or("No project open")?;
    ...
    let items = stmt.query_map(rusqlite::params![arc_id], |row| {
        Ok(EventNodeInfo {
            id: row.get(0)?, arc_id: row.get(1)?, chapter_number: row.get(2)?, event_type: row.get(3)?,
            summary: row.get(4)?, cause_refs: row.get(5)?, effect_refs: row.get(6)?, participants: row.get(7)?,
            impact_scope: row.get(8)?, status: row.get(9)?,
        })
    }).map_err(|e| e.to_string())?
    .collect::<Result<Vec<_>, _>>()
    .map_err(|e| e.to_string())?;
