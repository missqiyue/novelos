...
mod benchmark_tests {
    ...
    fn bench_compiler_2000_words() {
        ...
        let rules: Vec<CanonRuleForCompiler> = (0..50)
            .map(|i| CanonRuleForCompiler {
        ...
        let chars: Vec<CharacterForCompiler> = (0..30)
            .map(|i| CharacterForCompiler {
                name: format!("角色{}", i),
        ...
        let fs_items: Vec<ForeshadowForCompiler> = (0..20)
            .map(|i| ForeshadowForCompiler {
                id: format!("fs{}", i),
        ...
        println!(
            "Compiler benchmark: {}ms for {} chars, {} rules, {} characters, {} foreshadows",
            ms,
    ...
    fn bench_compiler_10000_words() {
        ...
        let rules: Vec<CanonRuleForCompiler> = (0..100)
            .map(|i| CanonRuleForCompiler {
                rule_name: format!("规则{}", i),
        ...
        assert!(
            ms < 2000,
            "Compiler took {}ms for 10k words, expected under 2s",
            ms
        );
        assert!(result.stats.word_count > 5000);

        println!(
            "Compiler 10k benchmark: {}ms for {} chars",
            ms, result.stats.word_count
        );
    }
    ...
    fn bench_state_machine() {
        ...
        println!(
            "Pipeline benchmark: 10k evaluations in {}ms ({}μs each)",
            ms,
            ms as f64 / 10.0
        );
    }
