...
// ─── Types for parsed outline sections ───
...
interface ChapterOutlineSections {
  开篇场景: string;
  情节点列表: PlotPoint[];
  角色出场: string[];
  关键对话: string[];
...
function parseOutlineContent(content: string): ChapterOutlineSections | null {
  try {
    // Try parsing as JSON first
    const parsed = JSON.parse(content);
    // Check if it has the expected structure
    if (parsed && typeof parsed === "object") {
      // The backend may return the outline under a "result" key or nested JSON
      const data = parsed.result || parsed;
      return {
        开篇场景: data.开篇场景 || data.opening_scene || "",
        情节点列表: Array.isArray(data.情节点列表)
          ? data.情节点列表
          : Array.isArray(data.plot_points)
            ? data.plot_points
            : [],
        角色出场: Array.isArray(data.角色出场)
          ? data.角色出场
          : Array.isArray(data.characters_appearing)
            ? data.characters_appearing
            : [],
        关键对话: Array.isArray(data.关键对话)
          ? data.关键对话
          : Array.isArray(data.key_dialogues)
            ? data.key_dialogues
            : [],
        转折点: data.转折点 || data.turning_point || "",
        章末状态: data.章末状态 || data.chapter_end_state || "",
      };
    }
  } catch {
    // Not JSON, try to extract sections from markdown/text
